...
每則 Message SHALL 記錄 `direction`（`inbound` 客人傳入 / `outbound` 客服回覆）與發送者（`sender_type` + `sender_id`）。

#### Scenario: 客人傳入訊息
- **WHEN** 平台傳來客人的訊息
- **THEN** 系統 SHALL 建立 direction=`inbound`、sender_type=`contact` 的 Message

#### Scenario: 客服回覆訊息
- **WHEN** 客服透過系統回覆
- **THEN** 系統 SHALL 建立 direction=`outbound`、sender_type=`agent` 的 Message
